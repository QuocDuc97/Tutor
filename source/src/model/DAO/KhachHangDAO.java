...
public class KhachHangDAO {
	...
	public String sinhMaKH() {
		String maKH = "KH" + DatabaseConnect.sinhMa("id", "KhachHang");
		return maKH;
	}
	...
	public KhachHang findCustomer(String code) {
		DatabaseConnect.getConnect();
		String sql = "Select * from KhachHang a where a.id=?";
	...
	public void updateCustomer(KhachHang customer, String id) {
		DatabaseConnect.getConnect();
		String sql = "Update KhachHang set hoten =?,email=?, gioitinh=?,sodienthoai=?,diachi=? where id='" + id + "'";
		PreparedStatement pstm;
		try {
			pstm = DatabaseConnect.conn.prepareStatement(sql);
